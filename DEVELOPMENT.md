...
postbase/
├── apps/
│   └── web/                   ← Main Next.js app (everything lives here)
│       ├── src/
...
├── api/
│   ├── auth/[projectId]/[...nextauth]/   ← Multi-tenant NextAuth handler
│   └── dashboard/
│       ├── projects/route.ts
...
- **API Keys**: generated with nanoid
  - Anon key: `pb_anon_<nanoid>`
  - Service role key: `pb_service_<nanoid>`
  - Validated via `validateApiKey()` in `lib/auth/keys.ts`
