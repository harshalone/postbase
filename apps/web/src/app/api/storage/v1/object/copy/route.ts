/**
 * POST /api/storage/v1/object/copy
...
export async function POST(req: NextRequest) {
  ...
  const [srcObj] = await db
    .select()
    .from(storageObjects)
    .where(and(eq(storageObjects.bucketId, bucket.id), eq(storageObjects.name, sourceKey)))
    .limit(1);

  if (!srcObj) return Response.json({ error: "Source object not found" }, { status: 404 });
  ...
  await db.insert(storageObjects).values({
    ...
    ownerId: srcObj.ownerId,
  });

  return Response.json({ data: { path: destinationKey } });
}
